...
async def email_part(app,l1_name, component_name, signal_name, value, update_date, alert_status,conn):
    supervisor_row = await conn.fetchrow("SELECT email FROM supervisor_email LIMIT 1")
    operator_row = await conn.fetchrow(
        "SELECT email, operator_name FROM operator_email WHERE machine_name = $1", l1_name
    )

    supervisor_email = supervisor_row["email"] if supervisor_row else None
    operator_email = operator_row["email"] if operator_row else None
    operator_name = operator_row["operator_name"] if operator_row and "operator_name" in operator_row else "the assigned operator"

                    # Compose email for supervisor
    if supervisor_email:
        supervisor_msg = EmailMessage()
        supervisor_msg["From"] = "hemanths02102003@gmail.com"
        supervisor_msg["To"] = supervisor_email

        if alert_status == "Emergency":
            print("🚨🚨🚨Sending Emergency Alert to Supervisor")
            supervisor_msg["Subject"] = f"🚨 Emergency Alert: {l1_name}"
            supervisor_msg.set_content(f"""
    Dear Supervisor,

    An emergency status has been detected on the following machine:

    • Machine Name: {l1_name}
    ...
    The assigned operator ({operator_name}) has been informed regarding this issue.
    ...
    Admin | NIAMT
            ...
            print("✅✅✅Sending Emergency Alert to Supervisor")
            supervisor_msg["Subject"] = f"✅ {l1_name} is back to normal"
            supervisor_msg.set_content(f"""
    Dear Supervisor,

    The following machine has returned to normal operating conditions:
    ...
    Admin | NIAMT
        ...
        await send(
            ...
            port=465,
            username="hemanths02102003@gmail.com",
            password="yxfrejyztfdekclw",
            use_tls=True
        )

    # Compose email for operator only for Emergency
    if alert_status == "Emergency" and operator_email:
        print("🚨🚨🚨Sending Emergency Alert to Operator")
        operator_msg = EmailMessage()
        operator_msg["From"] = "hemanths02102003l@gmail.com"
        operator_msg["To"] = operator_email
        operator_msg["Subject"] = f"⚠️ Urgent Check Required: {l1_name}"

        operator_msg.set_content(f"""
    Dear {operator_name},

    An emergency has been detected on your assigned machine. Please review the details below:
    ...
    try:
        ...
        for document in documents:
            ...
            async with pool.acquire() as conn:
                ...
                previous_status_row = await conn.fetchrow('''
                    SELECT status FROM alert_entry
                    WHERE l1_name = $1 AND signalname = $2
                    ORDER BY created_at DESC LIMIT 1
                ''', l1_name, signal_name)
                alert_status = "Emergency" if status in ["Emergency", "Tolerance"] else "Not_Emergency"
                insert_required = previous_status_row is None or previous_status_row["status"] != alert_status
                if insert_required:
                    # print("Inserted ✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅")
                    # print("STATUS CHECK:", status, "→", alert_status)
                    await conn.execute('''
                        INSERT INTO alert_entry (
                            l1_name, component_name, signalname, value, status
                        ) VALUES ($1, $2, $3, $4, $5)
                    ''', l1_name, component_name, signal_name, value, alert_status)

                    await email_part(app,l1_name, component_name, signal_name, value, update_date, alert_status, conn)
    except Exception as e:
